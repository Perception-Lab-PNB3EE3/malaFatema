...
>
> For the mental rotation task, participants will be shown three images in one trial one is the original 3D block image and the other two will be rotated at either a 45° or 90° angle. Out of the 2 options, participants will have to choose which rotated image matches the original image. The primary goal of the task is to assess how the urban or rural priming condition may influence participants’ performance on mental rotation tasks
>
> After participants complete the the trials, they will have to complete a demogrpahic survey stating their age and the type of environment that they grew up in (urban vs. rural). This information will be used in the exploratory analysis. 
...
> The software program G*Power was used to conduct a power analysis. Our goal was to achieve a power of 0.80, with an alpha of 0.05, and to detect a medium effect size (f = 0.25). These measures were determined based on standard conventions in psychological research. The power analysis indicated that a sample size of 34 participants would be enough.  
...
> In addition to the primary hypothesis, we will also ask the participants what type of environment they grew up in (urban vs rural) to see the relationship between environment background and performance on the task. 
